...
const Modal = ({ show, message, onClose }) => {
  ...
  return (
    ...
    >
      <div
        style={{ marginBottom: '10px', fontSize: '16px', textAlign: 'center' }}
      >
        {message}
      </div>
      <div
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          'http://127.0.0.1:8000/api/v1/sales-forecasting/combined-sales',
        );
        if (!response.ok) {
        ...
        const meanActualSales =
          actualSalesData.reduce(
            (sum: any, record: { total_sales: any }) =>
              sum + record.total_sales,
            0,
          ) / actualSalesData.length;
        if (predictedSalesData.total_sales < meanActualSales * (1 - 0.1)) {
          Swal.fire({
            title: 'Warning!',
            html: 'The predicted sales for the upcoming month are significantly lower than the average.',
            icon: 'warning',
          });
          //   setModalMessage('The predicted sales for the upcoming month are significantly lower than the average.');
          setModalVisible(true);
        } else if (
          predictedSalesData.total_sales >
          meanActualSales * (1 + 0.1)
        ) {
          Swal.fire({
            title: 'Warning!',
            html: 'The predicted sales for the upcoming month are significantly higher than the average.',
            icon: 'warning',
          });
        ...
        const transformedData = rawData.map(
          (item: { sale_year: any; sale_month: any; total_sales: any }) => ({
            x: `${item.sale_year}-${item.sale_month}`,
            y: item.total_sales,
          }),
        );
  ...
  return (
    <>
      ...
      <div className="col-span-12 rounded-sm border border-stroke bg-white px-5 pt-7.5 pb-5 shadow-default dark:border-strokedark dark:bg-boxdark">
        ...
        <div ref={chartRef}>
          <h1
            className="inline-block px-2 py-1 bg-blue text-center"
            style={{ color: '#15cfd1', marginLeft: 400, fontSize: 20 }}
          >
        ...
        <button
          onClick={downloadPDF}
          className="inline-block px-2 py-1 bg-blue text-center"
          style={{ color: '#15cfd1', marginTop: 30, marginLeft: 1000 }}
        >
          Download PDF
        </button>
      </div>
